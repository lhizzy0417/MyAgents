...
| `turn-lifecycle.ts` | turn completed/success flags、`TurnFinalizationGate`、turn start time、usage/context usage state |
...
| `transcript-persistence.ts` | in-memory `SessionMessage[]`、persisted runtime usage totals、assistant turn append + SessionStore save + metadata preview/context update |
| `interactive.ts` | permission / AskUserQuestion pending state、active IM request id、inbox/watch reply metadata；permission response delivery 成功后才 consume/delete |

Facade 仍执行跨 owner 编排：调用 runtime process、广播 SSE、调 IM/inbox/watch registry、做 analytics/title hook。Queue owner 不调用 runtime；lifecycle owner 不吞 stop cleanup；content owner 不向 facade 暴露 live mutable content refs，tool/subagent/attachment patch 走命名 API；transcript owner owns assistant turn append + SessionStore write path；persisted JSON shape 不变。
...
`turn_complete` / `session_complete` 触发的 finalization 在第一个 `await` 前同步 snapshot inbox/watch meta、context usage、content blocks ref、assistant text。随后等待 tool attachment in-flight saves，再序列化为 `JSON.stringify(ContentBlock[])` 写入 SessionStore——与 builtin runtime 格式一致，前端 `TabProvider.tsx` 的 JSON 解析路径直接复用。`TurnFinalizationGate` 在 `turn-lifecycle.ts`，`waitForExternalSessionIdle()` 必须等待 finalization settle 后才允许 cron/IM/injected caller 读取最新 assistant text。
