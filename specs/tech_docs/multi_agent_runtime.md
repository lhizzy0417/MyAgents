...
`agent-session.ts` 是 builtin SDK 的 public facade，`session-engine/builtin-adapter.ts` 只委托该 facade。Phase6 后，builtin 内部 mutable state 的真实 owner 是 `src/server/builtin-session/`：
...
Route modules、`SessionEngine` adapters 不直接 import `builtin-session/*`；新增 route-facing 能力仍先接 `SessionEngine`，再由 builtin adapter 调 `agent-session.ts` facade。`runtime-boundary.unit.test.ts` 对 route/session-engine/builtin-session 目录做边界扫描，并拦截 `agent-session.ts` 重新 direct-write owner state。`external-session.ts` 仍是 external runtime 内核；Phase6 没有抽 builtin/external 通用 lifecycle framework。
