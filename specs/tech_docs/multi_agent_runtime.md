...
### `RuntimeEnvPolicy.proxy` 三档语义（`env-utils.augmentedProcessEnv`）
...
| `'terminal'` | 剥掉继承的 proxy var，恢复用户 interactive shell 在 `~/.zshrc` / `~/.bashrc` 里 export 的（warmup 时 `shell.ts::warmupShellPath` 抓的 8 个 var：`HTTP_PROXY`/`HTTPS_PROXY`/`ALL_PROXY`/`NO_PROXY` × 大小写） | 用户终端能调某个 endpoint 但 MyAgents 里调不到（issue #194 原始场景） |
| `'direct'` | 剥掉所有 proxy var | Clash TUN / transparent proxy / VPN 等系统层路由网络的用户 |
