...
`FileIndexManager::search` 在进程内 slot 为空时优先打开磁盘已有 index + manifest；只有 index/manifest 缺失、版本不匹配或损坏时才重建。这样应用重启后的第一次文件搜索可以先返回上次可用结果，而不是无条件重新读完整工作区。
...
冷工作区仍然需要首次全量建索引；已有磁盘 index 的工作区会先显示可能稍旧的结果，再在后台收敛到最新文件系统状态。前端不在 Tab mount 或空搜索模式下预热/刷新索引，避免重 IO/CPU 工作排在用户真实 query 前面。
...
并发模型：全局 map 只短暂锁住以取得 `workspace → slot`；真正的刷新 / 冷建 / 搜索在该 workspace 的 slot 锁内执行，并由 `SearchEngine` 包进 `tokio::task::spawn_blocking`。后果：同一 workspace 的 refresh/search 仍保持串行一致性，不同 workspace 互不排队，也不会把同步 IO/CPU 工作压在 async runtime 线程上。
...
`cmd_invalidate_workspace_index` 硬删内存中的 `WorkspaceFileIndex` 和对应磁盘目录，下次搜索 / refresh 触发冷路径。保留为硬重置出口（schema 迁移、索引损坏恢复），常规场景走 `refresh_or_create`。
...
| 重启后第一次文件搜索仍冷建 | 文件区显示长时间“搜索中” | 工作区 index 缺少 `.file_index_manifest.json` 或 schema 版本不匹配 | 重新建索引后会写 manifest；后续重启走持久 index |
