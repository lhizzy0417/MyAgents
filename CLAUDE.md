...
| 前端任意 px 字号（`text-[13px]` / `text-[12px]`） | 字阶漂移：12px/14px 幽灵字阶曾长到 ~700 处 → 跨场景"字号大小不一"（用户投诉根因，PRD 0.2.34 清零） | 字阶 utility：`text-xs`(11)/`text-2sm`(12)/`text-sm`(13)/`text-md`(14)/`text-base`(16)/`text-lg/xl/2xl/3xl`，定义在 index.css `@theme`。**注意 text-sm=13px、text-xs=11px，与 Tailwind 官方值不同**。离阶展示型字号（品牌字）先在 DESIGN.md 立档再单行 disable | eslint (`src/renderer/**`) |
